...
@pytest.fixture
def order_service(mock_order_repo, mock_trade_repo):
    return OrderService(mock_order_repo, mock_trade_repo)
...
def test_create_order_rejected_insufficient_balance(order_service, mock_order_repo, mock_trade_repo):
    order_data = OrderCreate(
        broker="IBKR",
        symbol="DBS",
        order_type="LIMIT",
...
def test_create_order_filled(order_service, mock_order_repo, mock_trade_repo):
    order_data = OrderCreate(
        broker="IBKR",
        symbol="DBS",
        order_type="LIMIT",
...
def test_create_order_partial_fill(order_service, mock_order_repo, mock_trade_repo):
    order_data = OrderCreate(
        broker="IBKR",
        symbol="DBS",
        order_type="LIMIT",
...
def test_create_order_non_limit_type_fails():
    order_service = OrderService(Mock(), Mock())
    order_data = OrderCreate(
        broker="IBKR",
        symbol="DBS",
        order_type="MARKET",
